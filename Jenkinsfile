...
pipeline {
    ...
    stage('SonarQube Inspection') {
        steps {
            withSonarQubeEnv('SonarQube') { 
                ...
                -Dsonar.host.url=http://13.36.209.78:9000 \
                -Dsonar.login=1a6e28e61ee472cdd56bd8ee7b691b5e11c1e37c
                """
                }
            }
        }
    ...
    stage('Deploy to Development Env') {
        ...
        steps {
            withCredentials([usernamePassword(credentialsId: 'Ansible-Credential', passwordVariable: 'PASSWORD', usernameVariable: 'USER_NAME')]) {
                sh "ansible-playbook -i ${WORKSPACE}/ansible-config/aws_ec2.yaml ${WORKSPACE}/deploy.yaml --extra-vars \"ansible_user=ansibleadmin ansible_password=ansibleadmin hosts=tag_Environment_$HOSTS workspace_path=$WORKSPACE\""
            }
        }
    }
  ...
  post {
    always {
        ...
        slackSend channel: '#hameed-cicd-pipeline-alerts', //update and provide your channel name
